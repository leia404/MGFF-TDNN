...
data=/mnt/data/voxceleb
exp=/mnt/data/voxceleb/exp
exp_name=mgff_tdnn
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
  ...
  torchrun --nproc_per_node=$num_gpu $speakerlab/bin/train.py --config conf/mgff_tdnn.yaml --gpu $gpus \
           --data $data/train_feat_npy/npy.csv --noise $data/musan/wav.scp --reverb $data/rirs/wav.scp --exp_dir $exp_dir
fi
